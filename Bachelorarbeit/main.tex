...
\caption{PIConGPU simulation of laser wakefield acceleration. Displayed are the electrons, colored according to their energy $E$, the electric field of the laser pulse $E_x$ and the on-axis accelerating field $E_y$ in blue.}
\label{LWFA}
\end{figure}
...
LWFA works as follows: A laser is focused into a gas, usually hydrogen, where it needs to be intense enough to ionize the gas and push the electrons out of the way. In figure \ref{LWFA} this happens on the right of the laser pulse. Behind the laser they come back together which produces extreme electric fields on the order of several 100GV/m (see \cite{LWFAd}) in the propagation direction of the laser. This accelerating field can be seen in figure \ref{LWFA} in blue. Because these fields can become up to 3 orders of magnitude stronger than a traditional radiofrequency accelerator cavity they can accelerate electrons to multiple GeV over a distance of just centimeters \cite{LWFAd}. Figure \ref{LWFA} shows the electrons with a color gradient to show their energy. The yellow region is where the electron bunch is currently being accelerated by riding on the laser wakefield.

\section{The problem of Dephasing}
LWFA is known to be facing three major problems: Depletion, Diffraction and Dephasing. Depletion happens, when the laser pulse driving the wake has given so much of its energy to the electrons that it can no longer drive the wake. Diffraction is the laser pulse getting out of focus and therefore losing the wake. Dephasing describes the process by which electrons are accelerated to velocities over the speed of light in the plasma. Because of the high fields in the wake of the laser pulse this happens after a few centimeters of acceleration. The electrons can now slowly approach the laser pulse until the field in the propagation direction reverses and no longer accelerates the electrons and instead decelerates them. In figure \ref{LWFA} this reversal can be seen to the left of the laser pulse. This effect puts an upper limit to the energy an electron can leave the acceleration region with. A common solution is to accelerate the electrons in multiple stages to increase the energy but this is complicated and inconvinient.\\
Debus et al \cite{LWFAd} have shown that it is possible to solve Dephasing (and also Depletion) by using two seperate laser pulses with tilted wave fronts.
...
The following additional python modules were developed for this work. For a full documentation please go to the modules themselves on github.
...
\section{Testing different propagators}
...
\subsection{\pyth{AngularSpectrumPropagator}}


\subsection{\pyth{FresnelChirpZPropagator}}
...
All the code used in this work that I have written and mentioned here can be found on github at 
