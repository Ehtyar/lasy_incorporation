...
Abstract \\
English: Laser wakefield acceleration is a novel, compact method to accelerate electrons. Because the laser is moving slower than $c$ in the medium but the accelerated electrons approach $c$, the method faces the problem of dephasing. One proposed solution is the so called flying focus laser. It is generated using an axiparabola and a radial group delay echelon.\\
A flying focus laser has a complicated shape and phase structure and is, therefore, currently not available in PIConGPU. However, the Lasy library can simulate a laser pulse interacting with optical elements and can, therefore, generate a flying focus pulse. This work combines the two and makes laser pulses generated in Lasy available in PIConGPU. It also shows, that this technique works with Gaussian laser pulses. The properties of the flying focus laser, namely the option to arbitrarily set its speed, could unfortunately not be recreated.\\
...
\end{equation}
According to Ambat et al \cite{Ambat:2023} the beam waist, the radius at which field values fall to $1/e$ of their maximum, between $f_0<z<f_0+\delta$ should follow roughly the relation
\begin{equation}
...
\caption{A Gaussian laser pulse propagating into the PIConGPU simulation volume.}
\label{p_g_zr1}
\end{figure}
\begin{figure}[t!]
\centering
\includegraphics[width=0.7\textwidth]{Images/pic_parabol_zr2.png}
\caption{A Gaussian laser pulse propagated one Rayleigh length after the parabola focus in PIConGPU.}
\label{p_g_zr2}
\end{figure}
\begin{figure}[t!]
\centering
