...
\begin{titlepage}
 \begin{tabularx}{\linewidth}{X}
  ...
  \begin{singlespace}\begin{center}\large
   ...
   und\\Institut für Strahlenphysik\\
   Helmholtz Zentrum Dresden Rossendorf \\ 2025
  \end{center}\end{singlespace}
 \end{tabularx}
\end{titlepage}
...
Eingereicht am 19. Dezember 2025\vspace{1.5em}
...
\section*{Abstract}
\paragraph*{English:} Laser wakefield acceleration is a compact method to accelerate electron bunches to relativistic energies. These accelerators face a problem called "dephasing", where due to the reduced velocity of the laser in the plasma medium the electrons can only be accelerated over limited distances. One proposed solution to this problem is the so called flying focus laser. Experimentally, it is generated using an axiparabola and a radial group delay echelon.\\
To study the laser plasma interaction with flying focus lasers, Particle-in-Cell simulation codes, such as PIConGPU, are used. A flying focus laser has a complicated shape and phase structure and, therefore, was not available in PIConGPU. However, the Lasy library can simulate a laser pulse interacting with optical elements and can, therefore, model a flying focus pulse. This work combines these two and makes flying focus pulses generated in Lasy available in PIConGPU. Furthermore, this technique also enables modeling other laser pulses in PIConGPU via Lasy and the technique has been validated with both Gaussian and flying focus lasers. For the latter, inconsistencies between predicted and measured laser velocities and beam waists were discovered and are extensively discussed in this work.

\vspace{\fill}
\section*{Abstract}
\paragraph*{Deutsch:} "Laser-Wakefield-Acceleration" ist eine kompakte Methode um Elektronen zu relativistischen Energien zu beschleunigen. Solche Beschleuniger haben ein Problem, das "Dephasing" genannt wird, weil die Elektronen wegen der reduzierten Geschwindigkeit des Lasers im Plasma nur über begrenzte Strecken beschleunigt werden können. Eine mögliche Lösung für dieses Problem ist der so genannte "Flying-Focus-Laser". Dieser kann im Experiment mithilfe einer Axiparabel und eines radialen Verzögerungs-Echelons erzeugt werden.\\
Um die Interaktionen zwischen Laser und Plasma mit dem Flying-Focus-Laser genauer zu untersuchen nutzt man "Particle-in-Cell" Simulationen wie PIConGPU. Ein Flying-Focus-Laser hat eine komplizierte Form und Phasenstruktur und war deshalb in PIConGPU bisher nicht verfügbar. Mit der Python-Bibliothek Lasy kann man allerdings einen Laserpuls simulieren, der mit optischen Elementen interagiert, und dadurch einen Flying-Focus-Laser modellieren. Diese Arbeit kombiniert die beiden Methoden und macht den in Lasy erzeugten Flying-Focus-Laser in PIConGPU verfügbar. Außerdem ermöglicht die beschriebene Technik auch andere Laserpulse in PIConGPU via Lasy zu modellieren und wurde sowohl mit Gaußpulsen als auch mit dem Flying-Focus-Laser validiert. Für letzteren tauchten Inkonsistenzen zwischen vorhergesagten und beobachteten Lasergeschwindigkeiten und Strahlbreiten auf, die in dieser Arbeit ausführlich diskutiert werden.
...
\caption{PIConGPU simulation of laser wakefield acceleration. Displayed are the electrons, coloured according to their energy $E$, the electric field of the laser pulse $E_x$ and the on-axis accelerating field $E_y$ in blue. The laser pulse travels from left to right.}
\label{LWFA}
\end{figure}
Since the invention of LWFA by Tajima and Dawson \cite{LWFA}, compact accelerators have become a useful tool in radiation physics. They enable more labs to work with high energy electrons for their research. This in turn enables technologies using these electrons, for example free electron lasers which can generate laser pulses in otherwise unreachable frequency ranges. Our Group has used and researched them for a while now and is working on the cutting edge of this technology, see for example \cite{LWFA3}, \cite{LWFA2} and \cite{LWFA1}.\\
LWFA works as follows: A laser is focused into a gas such as hydrogen or helium, where it needs to be intense enough to ionize the gas and push the electrons out of the way. In figure \ref{LWFA} this happens on the right of the laser pulse, which travels here from left to right. Behind the laser they come back together which produces extreme electric fields on the order of several 100GV/m (see \cite{LWFAd}) in the propagation direction of the laser. This accelerating field can be seen in figure \ref{LWFA} in blue. Because these fields can become up to 3 orders of magnitude stronger than a traditional radio-frequency accelerator cavity, they can accelerate electrons to multiple GeV over a distance of just centimetres \cite{LWFAd}. Figure \ref{LWFA} shows the electrons with a colour gradient to show their energy. The yellow region is where the electron bunch is currently being accelerated by riding on the laser wakefield.

\section{The problem of Dephasing}
LWFA is known to be facing three major problems: Depletion, Diffraction and Dephasing. Depletion happens, when the laser pulse driving the wake has given so much of its energy to the electrons that it can no longer drive the wake. Diffraction is the laser pulse getting out of focus and therefore losing the intensity to drive the wake. Dephasing describes the process by which electrons are accelerated to velocities over the speed of light in the plasma. Because of the high fields in the wake of the laser pulse this happens after a few centimetres of acceleration. Then the electrons slowly approach the laser pulse until the accelerating field in the propagation direction reverses and no longer accelerates the electrons and instead decelerates them. In figure \ref{LWFA} this reversal can be seen to the left of the laser pulse. This effect puts an upper limit to the energy an electron can leave the acceleration region with. A common solution is to accelerate the electrons in multiple stages to increase the energy but this is complicated and inconvenient.\\
Debus et al \cite{LWFAd} have shown, that it is possible to solve Dephasing (and also Depletion) by using two separate laser pulses with tilted pulse fronts.
However, this is a complex setup. According to Palastro et al \cite{Palastro:2020} as well as Miller et al \cite{Miller:2023}, Dephasing could also be mitigated using a so called flying focus setup. It is made out of a radial group delay echelon and an axiparabola.
...
\end{equation}
According to Ambat et al \cite{Ambat:2023} the beam waist, the transversal radius at which field values fall to $1/e$ of their maximum, between $f_0<z<f_0+\delta$ should follow roughly the relation
\begin{equation}
	\label{axiparabola_wz}
	w(z)\approx\frac{\lambda_0f_0}{\pi R}\sqrt{\frac{\delta}{z-f_0}}.
\end{equation}
$z$ is here the distance from the axiparabola. This can of course only apply at sufficient distance from $f_0$, because otherwise $\lim_{z\rightarrow f_0}w(z) = \infty$. This is, of course, a problem for this formula but Ambat et al provide no further explanation. The time when the focus reaches a distance $z$ from the axiparabola is roughly:
\begin{align}
...
\caption{Full electric field of a Gaussian laser pulse at its focus, according to a Lasy simulation.Because of the time axis at the bottom, this pulse travels from right to left.}
\label{l_g_focus}
\end{figure}
\begin{figure}[t!]
\centering
\includegraphics[width=0.7\textwidth]{Images/lasy_parabol_saved.png}
\caption{The field that has been saved as an openPMD-compatible file after Lasy simulation of a Gaussian laser pulse, one Rayleigh length before the focus.}
\label{l_g_saved}
\end{figure}
...
\end{python}
It depends, like the optical index of plasma, only on the laser wavelength $\lambda_0$ and the electron density $n_e$.
...
The axiparabola that Lasy provides follows equation (\ref{sag_smartsev}) and, with that, Smartsev et al \cite{Smartsev:2019}. Because they are slightly different (see section \ref{test_axiparabola})the module also offers an alternative axiparabola implementation following Ambat et al \cite{Ambat:2023}.
...
\caption{Gauss pulse at its focus point according to the PIConGPU simulation. The artifacts to the side of the pulse are orders of magnitude less intense than the main peak of the pulse (see figure \ref{p_g_zr1} for reference).}
\label{p_g_focus}
\end{figure}
The module \pyth{showdata} contains functions to display metadata and data from openPMD-compatible files and calculate pulse parameters from this data. The main function of the module is
\begin{python}
...
showdata.show_lpeak()
\end{python}
These calculate the pulse waist and the longitudinal point of the peak, respectively. They each offer multiple methods to do this. One is calculating statistically, one is fitting a gaussian and, for \pyth{show_lpeak} specifically, one is finding the maximum. The statistical method is the standard.
...
\caption{A Gaussian pulse newly generated at the focus (left) and propagated $6z_R$ (right) in Lasy.}
\label{gauss_test}
\end{figure}
\begin{figure}[t!]
\centering
...
\caption{A Gaussian laser pulse in the near field of the parabolic mirror (left) and in the far field, one rayleigh length before the focus (right), simulated in Lasy.}
\label{l_g_parabola}
\end{figure}
...
From the parabolic mirror the laser propagated to one Rayleigh length $z_R$ before the focus by a distance of
\begin{equation}
\label{gausslength}
l=f-z_R=f-\frac{\pi w_0^2}{\lambda_0}\text{ with }w_0=\frac{f\lambda_0}{w\pi}.
\end{equation} Using the module \pyth{full_field}, the electric field of the laser was saved to an openPMD-compatible file. The goal was to feed it into PIConGPU one Rayleigh-length from the focus and then simulate it propagating through the focus point to one Rayleigh-length beyond it. In figure \ref{l_g_parabola} the pulse one Rayleigh-length before the focus is shown on the right. The saved field can be seen in figure \ref{l_g_saved}. To verify the Lasy propagation, the laser was also propagated with Lasy into the focus and out of it again by a Rayleigh-length. The beam waist at the focus, $w=23,51\mu$m, was consistent with the theoretical value of $w_0=23.48\mu$m in equation (\ref{gausslength}). The field at the focus is also shown in figure \ref{l_g_focus}.

\chapter{Lasy pulses in PIConGPU}
\label{picprop}
To use the saved laser pulse from the Lasy propagation in a PIConGPU simulation, PIConGPU's \textsf{incidentField.param} file needs to be ajusted to use \textsf{profiles::FromOpenPMDPulse}. The propagation direction in the file must be set to "z" and the mesh name to "E". In order for the file to fit on the GPUs, it is important to change the parameter \textsf{reservedGpuMemorySize} in the file \textsf{memory.param} to accomodate the file's size in addition to what is already required here.

\section{Courant-Friedrich-Levy (CFL) condition}
Dietrich \cite{fabia}, who implemented the \textsf{profiles::FromOpenPMDPulse}, showed that it is important, that in the file \textsf{simulation.param} the time step is the same (or very close to the same) as the time step in the saved laser pulse. (This can easily be achieved by setting the \pyth{forced_dt} option to that same time step when calling the function \pyth{full_field.laser_to_openPMD} to generate the laser file in Lasy.) However, because it may be neccessary to change the other parameters in the \textsf{simulation.param} file to be able to fit the laser pulse into a reasonable amount of cells, it is important to remember to check the CFL-condition. The size of the simulation cells ($\Delta x$, $\Delta y$, $\Delta z$) needs to be in the following relation with the time step $\Delta t$ (see \cite{PICdoc}):
\begin{equation}
...
If the deviation between the used and the ideal time step is less than 0.5\% the PIConGPU simulation will work without major problems \cite{CFLdoc}.

\section{Comparitative test with the Gauss laser}
\begin{figure}[t!]
\centering
\includegraphics[width=0.48\textwidth]{Images/pic_parabol_zr1.png}
\includegraphics[width=0.48\textwidth]{Images/pic_parabol_zr1_lin.png}
\caption{A Gaussian laser pulse propagating into the PIConGPU simulation volume. Left: On a symlog plot; right: on a linear plot. The artifacts behind the pulse are orders of magnitude less intense than the main peak of the pulse.}
\label{p_g_zr1}
\end{figure}
\begin{figure}[t!]
\centering
\includegraphics[width=0.7\textwidth]{Images/pic_parabol_zr2.png}
\caption{A Gaussian laser pulse propagated one Rayleigh length after the parabola focus in PIConGPU. The artifacts visible to the side of the pulse are still orders of magnitude less intense than the main peak of the pulse.}
\label{p_g_zr2}
\end{figure}
\begin{figure}[t!]
\centering
...
Figure \ref{p_g_zr1} shows the pulse as it enters the simulation volume. Of note: because PIConGPU  uses the y-axis as propagation direction, all figures from its simulations are labeled accordingly. It is still almost a Rayleigh length away from the focus point. Behind the pulse there are some artifacts from the field initialisation method. These are a known issue of the simulation, see \cite{PICartefact}. They can even move faster than light speed, as can be seen in figure \ref{p_g_focus} at the focus, where they have moved along significantly. Ignoring those for the moment, however, the waist of the pulse can be measured using \pyth{showdata.show_w()} and at the focus the value of $w=23.53\mu$m from the fitting of a Gaussian function and $w=23.64\mu$m calculated statistically (including the atrifacts visible in figure \ref{p_g_focus}) is similar to the calculated $w_0=23.48\mu$m from section \ref{gauss_gen}.
After a propagation of one Rayleigh length past the focus, the pulse looks like in figure \ref{p_g_zr2}. Here the waist is a lot larger again. It develops over time, as can be seen in figure \ref{p_g_ws}. Here the focus is in the middle and the plot shows the propagation by one Rayleigh length in each direction. It also shows the expected values from equation \ref{gauss_w} for comparison. They follow a very similar development, though towards the end the simulation artifacts skew the values to be larger. They can achieve this because the values of $w$ are calculated statistically here.\\
One can also see the shape of the wave fronts, always curved towards the focus point. At the focus point (figure \ref{p_g_focus}) they are flat.
...
\caption{A Gaussian laser pulse in the near field of the radial group delay echelon (left) and propagated 93 cm (right), simulated in Lasy.}
\label{l_rgd}
\end{figure}
\begin{figure}[t!]
\centering
\includegraphics[width=0.7\textwidth]{Images/lasy_rgd_ts.png}
\caption{The shape of a Gaussian laser pulse after the RGD, compared to the function $\tau_D(r)$, simulated in Lasy.}
\label{l_rgd_ts}
\end{figure}
\begin{figure}[t!]
\centering
...
\end{python}
The laser used in this test follows a Gaussian function longitudinally and a super-Gaussian function of order 6 transversally. In that way it both resembles the lasers used in previous papers such as Ambat et al \cite{Ambat:2023} or Oubrerie et al \cite{Oubrerie:2022} as well as a realistic shape for laser pulses used for LWFA.\\
Figure \ref{l_axiparabola} shows the laser pulse after the axiparabola is applied as well as after being propagated to the beginning of the focus. The full electric field at the focus is shown in figure \ref{l_a_focus}. Because the original pulse in this case was not a Gaussian pulse but a super-Gaussian pulse transversally, the shape at the focus is not a gaussian pulse either. It is instead close to a sinc function, as one would expect.
...
\label{p_a_wsts}
\end{figure}
Figure \ref{l_a_wsts} shows the results of a simulation where the laser propagated past the focus point through the focal region. Measured were the beam waist $w$, using the Lasy function \pyth{lasy.utils.laser_utils.get_w0}, and the arrival time of the pulse $t-\frac{z}{c}$, using the function \pyth{full_field.get_tpeak}. The former is calculated from the statistical standard deviation of the transversal intensity distribution, the latter is the statistical mean of the on-axis intensity distribution. Other methods were tried and give similar results. The two measured values are compared to theoretical calculations according to equations (\ref{axiparabola_wz}) and (\ref{axiparabola_tz}). The module \pyth{axiparabola_theory} (see section \ref{axiparabola_theory}) was used to calulate the values to compare to.\\
The measured values do not follow the theoretical calculations at all: While the beam waist $w$ is supposed to decrease over the focal region according to equation (\ref{axiparabola_wz}), it slowly increases instead. And wihile the arrival time of the pulse does decrease in relation to the speed of light as it should, it does this more slowly than described in equation (\ref{axiparabola_tz}).\\
As figure \ref{ambat_axisim} shows, Ambat et al were able to see the behavior described by the equations (\ref{axiparabola_wz}) (see image (c)) and (\ref{axiparabola_tz}) (see image (d)). This is clearly not what can be seen in the results here in figure \ref{l_a_wsts}.\\
...
In order to exclude another possible problem, the same propagation simulation was also run in PIConGPU. The results are shown in figure \ref{p_a_wsts}. The measured values are calculated from the field of the pulse statistically for this figure, similar to the values for figure \ref{l_a_wsts}, but other methods give similar results. The simulation methods are extremely different from one another: Lasy works in fourier space using angular spectrum propagation, PIConGPU numerically integrates the Maxwell equations on a grid using finite differences. Still, the results are of similar nature: The beam waist goes up instead of down and the focus does not move as expected either. The propagation method within the focal region is clearly not the problem here.\\
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.48\textwidth]{Images/axiparabola_de.png}
\includegraphics[width=0.48\textwidth]{Images/axiparabola_dde.png}
\caption{A numerical comparison of the left and right sides of equation (\ref{difeq}) for the shape of the axiparabola, using the sag function $s_f(r)$ from Ambat et al \cite{Ambat:2023}. Left: Both sides as absolute values. Right: The difference between the two sides.}
\label{axiparabola_de}
\end{figure}
...
For this reason figure \ref{axiparabola_shape} shows a comparison of the sag functions described in the different papers, the one used in Lasy and a normal parabolic mirror. They are all very similar. This makes it unlikely, that the shape of the axiparabola is the problem. Nonetheless, it is interesting to see, whether this shape fulfills the differential equation, that Smartsev et al \cite{Smartsev:2019} and Oubrerie et al \cite{Oubrerie:2022} used to derive the shape of the axiparabola:
\begin{equation}
\label{difeq}
s_f(r) - f(r) + \sqrt{\left(f(r)-s_f(r)\right)^2+r^2} = r\cdot \frac{\mathrm{d}s_f(r)}{\mathrm{d}r}
\end{equation}
Here $s_f(r)$ is the sag function describing the shape of the axiparabola and $f(r)$ is the function describing the dependence of the focus length on the radius $r$, the defining feature of the axiparabola.
As can be seen in figure \ref{axiparabola_de}, the two sides are almost identical. Therefore, the shape is likely to be correct.\\
In conclusion, the axiparabola does not work as intended or as described in previous papers. The reason(s) for this are still unknown, though multiple possibilities can be excluded.
...
\caption{The (transversally) super-Gaussian laser pulse in the near field of the combined elements (left) and in the far field at the focus (right); designed for $v_f=1.02c$ (top) and for $v_f=0.98c$ (bottom); simulated in Lasy.}
...
\label{l_f102_wsts}
\end{figure}
Combining the two elements, the radial group delay echelon and the axiparabola, and applying them to a laser pulse, one should obtain a flying focus laser. For this test the same initial laser with the super-Gaussian transversal shape as in section \ref{test_axiparabola} was used for the same reasons. Figure \ref{l_f_axiparabola} shows a visualisation of the laser pulse for desired focal velocities of $v_f=0.98c$ and $v_f=1.02c$ respectively.
The field shape is different to what it was before, as can be seen in figure \ref{l_f_focus}.
It is no longer symmetrical in the propagation direction. However, the two plots look like mirror images of each other. This is reasonable, as the peak of the pulse is supposed to be moving in opposite directions relative to the speed of light. Because the axiparabola cuts the pulse off at a specific radius $R$ and the RGD introduces sudden jumps as well, the diffraction artifacts are more pronounced than before but they should be mostly physical.\\
The figures \ref{l_f98_wsts} and \ref{l_f102_wsts} show the results of Lasy simulations with the flying focus assembly. As for the axiparabola, the beam waist $w$ and the arrival time $t-\frac{z}{c}$ are measured. Like in figure \ref{l_a_wsts} a consecutive and a pointwise separate simulation are shown. Of course the theoretical formula for the beam waist, equation (\ref{axiparabola_wz}), does not neccessarily apply for this setup. First of all, the beam waist is appearently larger in this case, according to the simulation. The curve is shown only for the lack of a better formula. However, the other diagram is of major importance here. The flying focus assembly is meant to make the focus point move at vacuum speed of light in the plasma of an LWFA chamber. For that, the focus time must move like the theoretical lines in the bottom plots of the two figures. This is clearly not the case. Still in this case the axiparabola did not work as intended. The RGD did not manage to fix the problem with it in the flying focus case.

\chapter{Summary and Outlook}

\section{Summary}
In conclusion, I was able to use Lasy and PIConGPU in conjunction in order to simulate laser pulses by first generating the pulse in Lasy, potentially sending it through optical elements and propagating it for some distances, save it to an openPMD compatible file that PIConGPU can read and finally simulate the laser from the file in PIConGPU. The previous chapters have shown, that this will correctly portray simple gauss pulses and that the translation from one simulation method to the other does not majorly change the outcome. I also managed to implement a working RGD echelon optical element that could be patched into the lasy library.\\
However, it was not possible to replicate the findings of other papers (Ambat et al \cite{Ambat:2023} etc., see section \ref{Theo_axiparabola}) regarding the axiparabola and its far field. Therefore it was also not possible to generate a flying focus laser as it has been described.

\section{Outlook}
In the future people will be able to use this new framework to use more complicated laser setups for their PIConGPU simulations. It will be possible to perform laser-plasma simulations with laser pulses, which have been shaped by optical elemtents first.\\
Some work remains to find the reason(s) for the differences between the simulations shown in this work and the findings of other groups. It could, for example, be useful to test different propagation methods in the Lasy simulations. Lasy offers a variety of options, that are currently in developement. In chapter \ref{lasyprop} is shown, that the standard propagator works for simple Gaussian pulses but it could be that the pulse here is to complicated for it. However, at their current implementation at the time of writing these alternative propagators dont support lasers in cylindrical coordinates and only cartesian coordinates are usable. This has the disavantage that the propagation computation is a lot more memory intesive, meaning it allows for fewer points transversally and longitudinally than previous simulations described in this work. This is why such tests are not included in this work. It might also be worthwhile to further investigate the axiparabola theory (see section \ref{Theo_axiparabola}). Mistakes here could also explain the problem.\\
It would also be interesting to further analyse the artifacts visible in figures \ref{p_g_focus}, \ref{p_g_zr1} and \ref{p_g_zr2}. For example: how much energy do they contain? How can they move faster than the speed of light?\\

\definecolor{ow}{RGB}{250, 180, 0}
\colorbox{black}{\textcolor{ow}{\texttt{(*) There's more to explore here.}}}

\section{Code availability}
All the code used in this work that I have written and mentioned here can be found on github at \cite{github}. Most images were created with the jupyter notebooks in that repository, using the modules mentioned in section \ref{modules}.
